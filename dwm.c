...
/* Check which layout is on a monitor */
#define ISBSTACK(M)             ((M)->lt[selmon->sellt] == (Layout *)&layouts[3])
#define ISMONOCLE(M)            ((M)->lt[selmon->sellt] == (Layout *)&layouts[2])
...
typedef struct Attachm Attachm;
...
static void focusmon(int dir, int focusstack);
static void focusin(XEvent *e);
static void focusstack(int pos);
...
static void sendmon(Client *c, Monitor *m, int attm);
...
static void tagmon(int dir, int attm);
static void tile(Monitor *m);
static void togglebar(const Arg *arg);
static void togglerbar(const Arg *arg);
...
struct Pertag {
	...
	int showrbars[LENGTH(tags) + 1];            /* display bar for the current tag */
};
...
void cycledirection(){
    attachdir = (attachdir >= LENGTH(attmeth) - 1 ? 0 : attachdir + 1);
    drawbars();
}

void setdirection(Arg *arg){
    attachdir = arg->i;
}

void attach(Client *c) { attmeth[attachdir].attach(c); }
...
createmon(void)
{
	...
	for (i = 0; i <= LENGTH(tags); i++) {
		...
		m->pertag->showrbars[i] = m->showrbar;
	}
...
drawbar(Monitor *m)
{
    ...
    if (!m->showrbar){
		...
		/* draw right text */
        etwr = TEXTW(estextr) - lrpad / 2 + 2; /* 2px extra right padding */
        drw_text(drw, m->ww - etwr - stw, 0, etwr, bh, lrpad / 2 - 2, estextr, 0);

		/* draw left text */
		etwl = TEXTW(estextl);
        x = 0;
		x = drw_text(drw, 0, 0, etwl, bh, 0, estextl, 0);

        if ((w = m->ww - etwr - stw - x) > bh) {
            if (m->sel) {
                drw_setscheme(drw, scheme[m == selmon ? SchemeSel : SchemeNorm]);
                drw_text(drw, x, 0, w, bh, lrpad / 2, m->sel->name, 0);
               if (m->sel->isfakefullscreen)
...
/* Returns the top slave window of a monitor (assuming there is one) */
Client *
getslave(Monitor *m)
{
	...
	for(p = NULL, c = m->clients; c && (i || !ISVISIBLE(c));
	    i -= ISVISIBLE(c) ? 1 : 0, p = c, c = c->next);

	return (c ? c : p);
}

/* Returns number of windows in monitor */
int 
nwinmon(Monitor *m)
{
    Client *c;
...
void pushdirection(const Arg *arg){
    ...
    if (!selmon->sel)
        return;
    if (selmon->sel->isfloating || ISFLOATING(selmon)){ 
        pushfloat(arg);

    /* In case there is only one visible window on the monitor */
    } else if (nwinmon(selmon) <= 1 || ISMONOCLE(selmon)){
        ...
        if (ismaster(selmon->sel)) {
            ...
            switch (dir) {
                ...
                default: pushstack(INC(-(dir - 2))); break;
            }
        } else {
            switch (dir) {
                case RIGHT: tagmon(1, 0); break;
                case LEFT: switchmaster(); break;
                default: pushstack(INC(-(dir - 2))); break;
            }
        }
        ...
        if (ismaster(selmon->sel)){
            switch (dir) {
                ...
                case LEFT:
                    focusmon(-1, 1);
                    break;
                default: focusstack(INC(-(arg->i - 2))); break;
            }
        /* In case a slave window is selected */
        } else {
            switch (dir) {
                case RIGHT:
                    focusmon(1, 0);
                ...
                case LEFT:
                    if (selmon->nmaster >= 1)
                        ...
                        focusmon(arg->i == DIR_RIGHT ? 1 : -1, arg->i == DIR_RIGHT ? 0 : 1);
                    break;
                default: focusstack(INC(-(arg->i - 2))); break;
            }
        }
        ...
        switch (dir) {
            ...
            case RIGHT: {
                ...
                if (ismaster(selmon->sel) == (nextpos + 1 <= selmon->nmaster) )
                    ...
                    focusmon(dir == RIGHT ? 1 : -1, arg->i == DIR_RIGHT ? 0 : 1);
                break;
            }
        }
    }
...
void
sendmon(Client *c, Monitor *m, int attm)
{
    attm = (!attm ? 0 : attm);
...
void
tagmon(int dir, int attm)
{
	if (!selmon->sel || !mons->next)
		return;
    Client *c = selmon->sel;
    Monitor *m = dirtomon(dir);
	sendmon(selmon->sel, m, attm);
    if (c)
        focus(c);
    arrange(m); arrange(selmon);
}


void
placePointer(Monitor *m)
{
    if (!pointerfocus)
        return;
    if (m->sel)
...
void
focusmon(int dir, int focusstack){
...
{
    if(selmon->sel)
        setfullscreen(selmon->sel, (!selmon->sel->isfullscreen && !selmon->sel->isfakefullscreen));
}

void togglefakefullscr(const Arg *arg){
    if (selmon->sel)
        selmon->sel->isfakefullscreen = !selmon->sel->isfakefullscreen && !selmon->sel->isfullscreen;
    drawbars();
}
...
view(const Arg *arg)
{
	...
	if (selmon->showrbar != selmon->pertag->showrbars[selmon->pertag->curtag])
		togglerbar(NULL);
